...
class PokeApiService {
  ...
  Future<List<PokemonListItem>> fetchPokemonList({int offset = 0, int limit = 50}) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('HTTP Error ${response.statusCode}: Failed to fetch Pokémon list');
    }
  }
  ...
  Future<PokemonDetail> fetchPokemonDetail(int id) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('HTTP Error ${response.statusCode}: Failed to fetch details for Pokémon #$id');
    }
  }
}
